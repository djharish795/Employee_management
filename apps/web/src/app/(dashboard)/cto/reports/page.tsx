  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [fieldWorkData, workReportsData] = await Promise.all([
        ...
        apiClient.get('/work-reports/team').then(r => r?.data?.data || r?.data || []).catch(err => { console.error('work-reports/team error:', err); return []; })
      ]);
    ...
    <div className="flex flex-col h-[calc(100vh-72px)] bg-slate-50 dark:bg-slate-950 animate-in fade-in duration-300">
      <div className="flex-1 overflow-y-auto p-6 md:p-8 max-w-[1600px] mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Operations Team Reports</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">View-only access to all operations team work and field requests.</p>
          </div>
        </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                  {(() => {
                    ...
                    return workReports.map((req, index) => (
