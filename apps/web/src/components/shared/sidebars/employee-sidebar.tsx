...
export function EmployeeSidebar() {
  ...
  const SidebarContent = ({ onNavigate }: { onNavigate?: () => void }) => (
    <>
      ...
      {/* User Footer */}
      <div className={`p-4 mt-auto border-t border-slate-200 dark:border-slate-800 ${collapsed ? 'flex justify-center' : ''}`}>
        {!collapsed ? (
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-3 overflow-hidden">
              <div className="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 flex items-center justify-center font-semibold text-sm flex-shrink-0 border border-slate-200 dark:border-slate-700">
                {initials}
              </div>
              <div className="overflow-hidden">
                <p className="text-sm font-medium text-slate-900 dark:text-white truncate">{userName}</p>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate capitalize">{displayRole}</p>
              </div>
            </div>
            <button
