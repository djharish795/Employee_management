...
export function usePermissions() {
  ...
  const canManageEmployees = ["SUPER_ADMIN", "CEO", "CHRO", "HR", "IT"].includes(role || "");
  ...
  const canManageSettings = ["SUPER_ADMIN", "IT", "CEO"].includes(role || "");
  const canManageOrg = ["SUPER_ADMIN", "CEO", "HR", "CHRO"].includes(role || "");
